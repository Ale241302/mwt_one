...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stderr.write(
                "ERROR: No se encontraron los modelos. Ajustar imports en seed_demo_data.py\n"
                "Paths esperados: apps.expedientes.models, apps.transfers.models\n"
            )
            return
        ...
        if options["flush"]:
            ...
            EventLog.objects.filter(expediente__brand="marluvas_demo").delete()
            PaymentLine.objects.filter(expediente__brand="marluvas_demo").delete()
            CostLine.objects.filter(expediente__brand="marluvas_demo").delete()
            ArtifactInstance.objects.filter(expediente__brand="marluvas_demo").delete()
            Expediente.objects.filter(brand="marluvas_demo").delete()
            Transfer.objects.filter(transfer_id__startswith="TRF-DEMO").delete()
            Node.objects.filter(node_id__startswith="DEMO-").delete()
            if HAS_LIQUIDATIONS:
                LiquidationLine.objects.filter(liquidation__period__startswith="DEMO").delete()
        ...
        ummie_gt, _ = LegalEntity.objects.get_or_create(
            entity_id="UMMIE-GT",
            defaults={
                "legal_name": "UMMIE Guatemala S.A.",
                "country": "GTM",
                "role": "DISTRIBUTOR",
                "relationship_to_mwt": "DISTRIBUTION",
                "frontend": "MWT_ONE",
        ...
        imporcomp_co, _ = LegalEntity.objects.get_or_create(
            ...
            defaults={
                "legal_name": "Imporcomp Colombia S.A.S.",
                "country": "COL",
                "role": "DISTRIBUTOR",
                "relationship_to_mwt": "DISTRIBUTION",
                "frontend": "MWT_ONE",
        ...
        # HELPERS
        # ══════════════════════════════════════════════════════════════
        def make_exp(client_entity, legal_entity, mode, status, days_ago, **kwargs):
            ...
            exp.refresh_from_db()
            self.stdout.write(f"  EXP {exp.pk} [{status}] — {client_entity.entity_id} ({mode})")
            return exp
        ...
        # ══════════════════════════════════════════════════════════════
        # 1. CERRADO — Sondel, happy path completo
        # ══════════════════════════════════════════════════════════════
        exp1 = make_exp(sondel_cr, mwt_cr, "FULL", "CERRADO", 90,
                        payment_status="PAID",
        ...
        # ══════════════════════════════════════════════════════════════
        # 2. TRANSITO — Sondel, reloj día 55
        # ══════════════════════════════════════════════════════════════
        exp2 = make_exp(sondel_cr, mwt_cr, "FULL", "TRANSITO", 60,
        ...
        # ══════════════════════════════════════════════════════════════
        # 3. EN_DESTINO — UMMIE GT, reloj día 78 ⚠
        # ══════════════════════════════════════════════════════════════
        exp3 = make_exp(ummie_gt, mwt_cr, "COMISION", "EN_DESTINO", 85,
        ...
        # ══════════════════════════════════════════════════════════════
        # 4. BLOQUEADO — Sondel, reloj día 82 🔴
        # ══════════════════════════════════════════════════════════════
        exp4 = make_exp(sondel_cr, mwt_cr, "FULL", "TRANSITO", 88,
                        is_blocked=True,
        ...
        # ══════════════════════════════════════════════════════════════
        # 5. REGISTRO — Imporcomp, recién creado
        # ══════════════════════════════════════════════════════════════
        exp5 = make_exp(imporcomp_co, mwt_cr, "COMISION", "REGISTRO", 3)
        make_artifact(exp5, "ART-01", payload={"po_number": "PO-CO-2026-015", "total": 8500.00}, days_ago=3)
        make_event(exp5, "expediente.created", 3)

        # ══════════════════════════════════════════════════════════════
        # 6. PRODUCCION — Sondel, en fábrica
        # ══════════════════════════════════════════════════════════════
        exp6 = make_exp(sondel_cr, mwt_cr, "FULL", "PRODUCCION", 30)
        ...
        # ══════════════════════════════════════════════════════════════
        # 7. PREPARACION — UMMIE GT
        # ══════════════════════════════════════════════════════════════
        exp7 = make_exp(ummie_gt, mwt_cr, "COMISION", "PREPARACION", 45)
        ...
        # ══════════════════════════════════════════════════════════════
        # 8. DESPACHO — Sondel, embarcando
        # ══════════════════════════════════════════════════════════════
        exp8 = make_exp(sondel_cr, mwt_cr, "FULL", "DESPACHO", 50,
        ...
        # ══════════════════════════════════════════════════════════════
        # 9. CANCELADO — Imporcomp
        # ══════════════════════════════════════════════════════════════
        exp9 = make_exp(imporcomp_co, mwt_cr, "COMISION", "CANCELADO", 40)
        ...
        # ══════════════════════════════════════════════════════════════
        # 10. CERRADO — UMMIE GT, modelo comisión pagado
        # ══════════════════════════════════════════════════════════════
        exp10 = make_exp(ummie_gt, mwt_cr, "COMISION", "CERRADO", 120,
                         payment_status="PAID",
        ...
        try:
            node_fiscal, _ = Node.objects.get_or_create(
                ...
                defaults={"name": "Almacén Fiscal CR", "node_type": "fiscal",
                          "legal_entity": mwt_cr, "status": "active"})
            node_own, _ = Node.objects.get_or_create(
                node_id="DEMO-OWN-WH-CR",
                defaults={"name": "Bodega MWT CR", "node_type": "owned_warehouse",
                          "legal_entity": mwt_cr, "status": "active"})
            node_fba, _ = Node.objects.get_or_create(
                node_id="DEMO-FBA-US",
                defaults={"name": "Amazon FBA USA", "node_type": "fba",
                          "legal_entity": mwt_cr, "status": "active"})
            ...
            trf1 = Transfer.objects.create(
                ...
                to_node=node_own,
                ownership_before=mwt_cr,
                ownership_after=mwt_cr,
...
║                                                                  ║
║  TRANSFERS: TRF-DEMO-001 (in_transit), TRF-DEMO-002 (planned)  ║
