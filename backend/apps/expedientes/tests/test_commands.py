...
from apps.expedientes.enums_exp import ExpedienteStatus, DispatchMode, PaymentStatus
...
class CommandAPITests(APITestCase):
    ...
    def test_c1_create_expediente(self):
        ...
        self.assertEqual(Expediente.objects.get(pk=res.data['expediente_id']).brand, 'NewBrand')
        self.assertEqual(len(res.data['events']), 1)
        self.assertIsNotNone(Expediente.objects.get(pk=res.data['expediente_id']).credit_clock_started_at)
    ...
    def test_c1_on_shipment_rule(self):
        ...
        res = self.client.post(url, data, format='json')
        self.assertIsNone(Expediente.objects.get(pk=res.data['expediente_id']).credit_clock_started_at)
    ...
    def test_c2_register_oc(self):
        ...
        self.assertIn(res.status_code, [status.HTTP_200_OK, status.HTTP_201_CREATED])
        self.assertTrue(ArtifactInstance.objects.filter(artifact_type='ART-01').exists())

    def test_c3_register_proforma(self):
        ArtifactInstance.objects.create(expediente=self.exp, artifact_type='ART-01', status='completed')
    ...
    def test_c8_register_freight_quote(self):
        self.exp.status = ExpedienteStatus.PREPARACION
        self.exp.save()
        ArtifactInstance.objects.create(expediente=self.exp, artifact_type='ART-05', status='completed')
