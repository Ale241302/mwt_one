...
from apps.factory_orders.models import factory_order
...
class TestAccrualT5(TestCase):
    ...
    def setUp(self):
        ...
        self.factory_order = factory_order.objects.create(
            client=self.client,
    ...
    def test_t5a_basic_accrual_invoiced(self):
        result = calculate_rebate_accrual(
            ledger=self.ledger, factory_order=self.factory_order
        )
    ...
    def test_t5c_idempotency(self):
        calculate_rebate_accrual(ledger=self.ledger, factory_order=self.factory_order)
        result2 = calculate_rebate_accrual(ledger=self.ledger, factory_order=self.factory_order)
...
class TestAccrualConcurrentT5F(TransactionTestCase):
    ...
    def test_t5f_concurrent_accrual_no_race_condition(self):
        factory_order1 = factory_order.objects.create(client=self.client, total_amount=Decimal("1000.00"))
        factory_order2 = factory_order.objects.create(client=self.client, total_amount=Decimal("2000.00"))
        errors = []

        def run_accrual(pf):
            try:
                calculate_rebate_accrual(ledger=self.ledger, factory_order=pf)
            except Exception as e:
                errors.append(e)

        t1 = threading.Thread(target=run_accrual, args=(factory_order1,))
        t2 = threading.Thread(target=run_accrual, args=(factory_order2,))
        t1.start()
        t2.start()
        t1.join()
        t2.join()
