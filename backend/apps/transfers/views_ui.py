...
class TransfersUIView(APIView):
    ...
    def get(self, request):
        ...
        qs = (
            Transfer.objects
            .select_related('from_node', 'to_node', 'from_node__legal_entity', 'to_node__legal_entity')
            .order_by('-created_at')
        )
        if estado:
            qs = qs.filter(status=estado.lower())
        transfers = qs[:100]
        nodes = Node.objects.select_related('legal_entity').filter(status='active').order_by('name')
        return Response({
