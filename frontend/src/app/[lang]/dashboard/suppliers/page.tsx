...
export default function SuppliersPage() {
  ...
  const fetchSuppliers = useCallback(async () => {
    ...
    try {
      ...
      const [sr, br] = await Promise.all([
        api.get(`/api/suppliers/?${query.toString()}`),
  ...
  const updateFilters = (newFilters: Record<string, string>) => {
    ...
    router.push(`/${lang}/dashboard/suppliers?${nextParams.toString()}`);
  };
      ...
      {loading ? (
        ...
        <div className="table-container">
          <table>
            ...
            <tbody>
              {suppliers.map((s) => (
                <tr key={s.id} className="group cursor-pointer hover:bg-surface-secondary" onClick={() => router.push(`/${lang}/dashboard/suppliers/${s.id}`)}>
                  <td>
