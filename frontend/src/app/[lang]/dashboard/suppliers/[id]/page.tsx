...
export default function SupplierDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [sr, cr, ar, catR, kr] = await Promise.all([
        ...
        api.get(`/api/suppliers/${id}/contacts/`),
        api.get(`/api/suppliers/${id}/agreements/`),
        api.get(`/api/suppliers/${id}/catalog/`),
        api.get(`/api/suppliers/${id}/performance/`),
      ]);
      setSupplier(sr.data);
      setContacts(cr.data);
  ...
  const handleSaveKpi = async () => {
    setSavingKpi(true);
    try {
      await api.post(`/api/suppliers/${id}/register_kpi/`, kpiForm);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-8">
        <button 
          onClick={() => router.push(`/${lang}/dashboard/suppliers`)}
