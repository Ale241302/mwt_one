    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6 p-3 rounded-xl" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
        ...
        {(["all", "green", "amber", "red"] as CreditLevel[]).map((level) => (
          <button
            key={level}
            onClick={() => setCreditFilter(creditFilter === level ? "all" : level)}
            className="btn btn-sm"
            style={{
              background: creditFilter === level ? "var(--surface-active)" : "transparent",
              color: "var(--text-primary)",
              border: `1px solid ${creditFilter === level ? "var(--border-strong)" : "var(--border)"}`,
            }}
          >
      ...
      {/* Pipeline Kanban — S9.1-06: cards son <button> para keyboard nav */}
      {viewMode === "pipeline" && (
              ...
              <div key={status} className="pipeline-column">
                <div className="pipeline-column-header">
                  ...
                  <span
                    ...
                    style={{ background: "var(--brand-accent)", color: "var(--brand-primary)", width: 22, height: 22, fontSize: 11, fontWeight: 700 }}
                  >
                    {items.length}
                  </span>
                </div>
                      ...
                      >
                        ...
                        <div className="pipeline-card-meta">
                          ...
                          <div className="flex items-center gap-1 ml-auto" title={`${exp.artifacts_done}/${exp.artifacts_total} artefactos`}>
                            ...
                            {(exp.artifacts_total || 0) > 0 && (
                              <span className="caption ml-1">{exp.artifacts_done || 0}/{exp.artifacts_total}</span>
                            )}
                          </div>
                        </div>
                      </button>
      ...
      {/* Table — S9.1-06: filas pasivas, botón explícito para navegar */}
      {viewMode === "tabla" && (
      ...
      {viewMode === "calendario" && (
        <div className="empty-state">
          <Calendar size={48} />
          <p>Vista calendario disponible en Sprint 10</p>
        </div>
      )}
    </div>
  );
