...
status: DONE — Ejecutado por Alejandro (AG-02/AG-07) · Aprobado CEO 2026-03-12
visibility: [INTERNAL]
...
**Precondición:** Sprint 1 DONE — 18 endpoints funcionales, outbox llenándose, credit_clock_started_at persistido en C1/C7.
...
## Decisión de infraestructura: Celery + Redis

Sprint 0 (PLB_ORCHESTRATOR §G) declara 6 contenedores incluyendo celery-worker y celery-beat, pero ENT_PLAT_MVP §B dice "MVP NO incluye Redis, Celery workers".

**Resolución Sprint 2:** Se agregan Redis + Celery worker al docker-compose como items explícitos de este sprint. Celery Beat ya existe desde Sprint 0 (declarado en LOTE_SM_SPRINT0 Item 1). Si Sprint 0 no lo incluyó realmente, se agrega aquí.

Stack después de Sprint 2:
- Existentes: django, postgres, nginx, minio, celery-beat
- Nuevos: redis, celery-worker

[PENDIENTE — CEO: confirmar que Sprint 0 efectivamente creó celery-beat. Si no, Item 1 de este sprint lo incluye.]
...
- **Criterio de done:**
  ...
  - [ ] Celery Beat corriendo (verificar si ya existe de Sprint 0, si no, crear)
...
- **Criterio de done:**
  ...
  - [ ] Día 75+: si `is_blocked=false` → ejecuta C17 BlockExpediente automático (`blocked_by_type=SYSTEM, blocked_by_id="credit_clock_75d"`) via services.execute_command. Emite `credit_clock.critical`.
  - [ ] Día 90+: emite `credit_clock.expired { expediente_id, days_elapsed: 90 }`. Si no estaba bloqueado (edge case), bloquea ahora (`blocked_by_id="credit_clock_90d"`). Ref §M2.
  - [ ] Task es idempotente: correr 2 veces el mismo día no duplica eventos ni bloqueos (verificar estado actual antes de actuar)
...
- Si expediente ya está bloqueado por credit_clock (blocked_by_id contiene "credit_clock"): no re-bloquear, no emitir evento duplicado
- Si evento credit_clock.warning ya existe para hoy para este expediente: no duplicar
- Cada evaluación registra el umbral más alto alcanzado, no todos los anteriores
...
- **Agente:** AG-02 API Builder
- **Dependencia previa:** Item 2 aprobado (services.py estable con evaluación)
...
- **Agente:** AG-06 QA
- **Dependencia previa:** Items 2-5 aprobados
...
11. Los 20 endpoints anteriores (Sprint 1) siguen funcionales (no regresión)
...
- Total: 20 endpoints (Sprint 1) + 2 nuevos (C19, C20) = 22 endpoints
...
Stamp: DONE — CERRADO · 7/7 items · 0 pendientes · Confirmado auditoría 2026-03-12
Tareas pasadas: ninguna.
Origen: Derivado de ENT_OPS_STATE_MACHINE v1.2.2 (FROZEN) + PLB_ORCHESTRATOR v1.2.2 (FROZEN) + priorización CEO 2026-02-27
