...
3. Verificar que `django_admin_log` no tiene restricción de FK activa que bloquee el swap — si la tiene, desactivarla primero

Alejandro en el **Paso 1 de Item 1** completa esta checklist antes de tocar `AUTH_USER_MODEL`. Si aparecen campos adicionales en apps propias, los agrega a `users/0002_migrate_ceo.py`.
...
Ejecutar en repo: `grep -rn "ForeignKey.*User\|auth.User" apps/` y listar todos los resultados. Arquitecto definió lista base en D-02 (EventLog, ArtifactInstance, Transfer). Si aparecen campos adicionales, agregarlos a `users/0002_migrate_ceo.py`.
...
- **Nota user_id:** Sprint 0 ya incluye `user_id` en el payload JWT. Sprint 8 **preserva** ese claim y añade `role`, `legal_entity_id`, `permissions[]` — sin remover ningún claim existente. `request.auth['user_id']` sigue disponible en Item 12 (Redis ownership) e Item 13 (ConversationLog).
...
**Criterio de done:**
- [ ] `MWTTokenObtainPairSerializer` reemplaza el serializer base en `urls.py`
...
kw:session_meta:{user_id}:{session_id} → Hash {owner_user_id, created_at}        TTL 2h sliding
```
...
if not session_exists and not meta_exists: # sesión nueva O recién limpiada
    with redis.pipeline(transaction=True) as pipe:
        ...
        pipe.hset(meta_key, mapping={"owner_user_id": user_id, "created_at": now_iso})
        pipe.expire(session_key, 7200)
        pipe.expire(meta_key, 7200)
        pipe.execute()
...
**Ownership:** al leer `meta_key`, verificar `owner_user_id == user_id del request`. Si no coincide → 403 antes de cualquier operación.
...
- [ ] Toda operación que toca ambas keys usa `pipeline(transaction=True)` — sin comandos Redis sueltos cuando se modifica el par
...
```
Post-respuesta FastAPI → `calculate_retention()` → INSERT `ConversationLog` usando `user_id` del claim (sin ORM lookup adicional).
Timeout: 5s → 503 descriptivo.

**Response de `/api/knowledge/ask/` — `session_id` siempre presente:**
El response JSON de este endpoint incluye siempre el `session_id` activo, tanto si vino en el request como si fue autogenerado:
```json
{
  "answer": "...",
...
- [ ] `user_id=1` lee sesión de `user_id=2` → 403
...
- [ ] `curl http://localhost:8001/status/` desde dentro de la red interna → 200 sin header (excepción explícita — ver Item 11)
- [ ] `curl http://localhost:8001/search/ -H "X-Internal-Token: test"` sin body → 422 (endpoint protegido requiere el header)
...
## Excluido de Sprint 8 — resuelto en Sprint 9

Los módulos UI pendientes (PLT-02, PLT-03, PLT-04, PLT-05, PLT-06, PLT-07) están congelados fuera de este sprint. Orden de ejecución, bloqueadores y estado canónico viven en **ENT_GOB_PENDIENTES** — no se duplican aquí.

Módulos ejecutados en Sprint 6 (Consola QR, Rana Walk mwt.one, go.ranawalk.com DNS): ver LOTE_SM_SPRINT6 DONE 2026-03-12.

---

## Lo que desbloquea este sprint

- **Sprint 9 UI Batch:** MWTUser completo habilita PLT-02 Usuarios UI. El resto de PLT (04,05,06,07) no depende de Sprint 8 pero se ejecuta en el mismo sprint por coherencia de entrega.
- Canal WhatsApp: webhook llama `/api/knowledge/ask/` con JWT
- n8n workflows: pueden llamar `/api/knowledge/search/`
- Indexación PDFs MinIO + `document_index` operativos (Sprint 9+)
- Portal B2B: base de usuarios lista

---

Stamp: DRAFT v3.10 — Post-auditoría ChatGPT Ronda 10 (4 fixes aplicados) — Pendiente aprobación CEO → FROZEN
